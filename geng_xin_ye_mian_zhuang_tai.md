...
在这个文件中，我们已经定义并初始化了页面的UI状态，要为当前的状态添加“更新操作”，只需要在该 ```funciton``` 的函数体添加对应的行为即可。而不管添加多少行为，我们必须保证**函数执行完毕后必须返回一个 全新的 ```Object``` 作为新的 ```State```**。
...
我们可以在文件 ```_assets/action.js``` 中定义 ```动作创建器（Action Creator）```。和 ```_assets/reducer.js``` 中定义 ```页面状态``` 时区分 ```应用状态（$appState）``` 和 ```UI状态（$uiState）``` 一样，我们的 ```动作``` 也分为 ```应用相关的动作（$appAction）``` 和 ```UI相关的动作（$uiAction）```。与之对应的，```动作创建器``` 也被分类为 ```应用相关的动作创建器``` 和 ```UI相关的动作创建器```。
